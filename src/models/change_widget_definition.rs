/*
 * Datadog API V1 Collection
 *
 * Collection of all Datadog Public endpoints.
 *
 * The version of the OpenAPI document: 1.0
 * Contact: support@datadoghq.com
 * Generated by: https://openapi-generator.tech
 */

/// ChangeWidgetDefinition : The Change graph shows you the change in a value over the time period chosen.



#[derive(Clone, Debug, PartialEq, Serialize, Deserialize)]
pub struct ChangeWidgetDefinition {
    /// List of custom links.
    #[serde(rename = "custom_links", skip_serializing_if = "Option::is_none")]
    pub custom_links: Option<Vec<crate::models::WidgetCustomLink>>,
    /// Array of one request object to display in the widget.  See the dedicated [Request JSON schema documentation](https://docs.datadoghq.com/dashboards/graphing_json/request_json)  to learn how to build the `REQUEST_SCHEMA`.
    #[serde(rename = "requests")]
    pub requests: Vec<crate::models::ChangeWidgetRequest>,
    #[serde(rename = "time", skip_serializing_if = "Option::is_none")]
    pub time: Option<Box<crate::models::WidgetTime>>,
    /// Title of the widget.
    #[serde(rename = "title", skip_serializing_if = "Option::is_none")]
    pub title: Option<String>,
    #[serde(rename = "title_align", skip_serializing_if = "Option::is_none")]
    pub title_align: Option<crate::models::WidgetTextAlign>,
    /// Size of the title.
    #[serde(rename = "title_size", skip_serializing_if = "Option::is_none")]
    pub title_size: Option<String>,
    #[serde(rename = "type")]
    pub _type: crate::models::ChangeWidgetDefinitionType,
}

impl ChangeWidgetDefinition {
    /// The Change graph shows you the change in a value over the time period chosen.
    pub fn new(requests: Vec<crate::models::ChangeWidgetRequest>, _type: crate::models::ChangeWidgetDefinitionType) -> ChangeWidgetDefinition {
        ChangeWidgetDefinition {
            custom_links: None,
            requests,
            time: None,
            title: None,
            title_align: None,
            title_size: None,
            _type,
        }
    }
}


